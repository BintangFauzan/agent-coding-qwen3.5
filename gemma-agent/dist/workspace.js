...
export function initWorkspace(argv = process.argv) {
...
export function validatePath(resolvedPath) {
    const root = getWorkspaceRoot();
    const normalizedResolved = normalize(resolvedPath);
    const normalizedRoot = normalize(root);
    if (normalizedResolved !== normalizedRoot &&
        !normalizedResolved.startsWith(normalizedRoot + sep)) {
        throw new Error(`Path '${resolvedPath}' is outside workspace '${root}'. Access denied.`);
    }
}
